...
inline constexpr std::size_t kLegacyLoginReply284MinimumPayloadSize = 36;
...
inline auto decode_legacy_login_reply_284_payload(std::span<const std::uint8_t> bytes)
  ...
  const auto parental_control_timer = reader.read_u32_le();
  const auto unknown2 = reader.read_bytes(8);
  const auto account_id = reader.read_u32_le();
  if (!parental_control_flag.has_value() || !parental_control_timer.has_value() ||
      !unknown2.has_value() || !account_id.has_value()) {
    return std::nullopt;
  }
  ...
  if (!reset_appearance.has_value() || !do_not_force_soga.has_value() ||
      !unknown5.has_value() || !unknown6.has_value() || !unknown7.has_value() ||
...
inline auto encode_login_reply_payload(const LoginReplyPayload& payload,
    ...
    -> std::vector<std::uint8_t> {
  if (legacy_login_reply_uses_284_layout(client_version)) {
    return encode_legacy_login_reply_284_payload(payload);
  }
