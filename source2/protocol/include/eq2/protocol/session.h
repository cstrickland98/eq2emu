...
inline auto encode_session_request(const SessionRequest& request)
  ...
  write_u32_be(bytes, 0, request.unknown_a);
  write_u32_be(bytes, 4, request.session);
  write_u32_be(bytes, 8, request.max_length);
  return bytes;
}
...
inline auto decode_session_request(std::span<const std::uint8_t> bytes)
  ...
  return SessionRequest{
      .unknown_a = read_u32_be(bytes, 0),
      .session = read_u32_be(bytes, 4),
      .max_length = read_u32_be(bytes, 8),
  };
}
...
inline auto encode_session_response(const SessionResponse& response)
  ...
  write_u32_be(bytes, 0, response.session);
  write_u32_be(bytes, 4, response.key);
  bytes[8] = response.unknown_a;
  bytes[9] = response.format;
  bytes[10] = response.unknown_b;
  write_u32_be(bytes, 11, response.max_length);
  write_u32_be(bytes, 15, response.unknown_d);
  return bytes;
}
...
inline auto decode_session_response(std::span<const std::uint8_t> bytes)
  ...
  return SessionResponse{
      .session = read_u32_be(bytes, 0),
      .key = read_u32_be(bytes, 4),
      .unknown_a = bytes[8],
      .format = bytes[9],
      .unknown_b = bytes[10],
      .max_length = read_u32_be(bytes, 11),
      .unknown_d = read_u32_be(bytes, 15),
  };
}
