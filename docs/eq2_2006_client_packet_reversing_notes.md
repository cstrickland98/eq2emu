...
| `0x1e6` | `0x004742c0` | start-broker command | Serializer `0x0094f974` writes three `u32` fields followed by one `u8`; EQ2Emu's 546 row keeps the broker flag bytes contiguous as five `u8`s after the first two words so existing send paths can set index 4. Handler resolves the `Market` UI and opens/configures broker state. |
| `0x1e7` | `0x0064dca0` + `0x00652960` | map-exploration command | Serializer `0x00952178` writes `u16` count followed by 0x0c-byte xyz records; handler resolves `Map` and iterates exploration/map region records, marking map UI state dirty when records change. |
...
| `0x1e9` | `0x00473f30` | spell-move-to-range-and-retry command | Serializer `0x0094fc4c` writes target id, spell id, range/float, then `string16` spell name; handler checks spell movement capability and routes spell id/range/target fields to the move-and-retry helper. |
| `0x1ea` | `0x00491b30` | update-player-mail command | Serializer `0x0095083e` writes `u8 action`, shared mail-attachment payload (`u32` coin fields, item/detail header, `u32` blob size plus bytes), then `u8 unread/notify` and `u32 mail_id`; handler resolves `Mail`, handles add/delete/read/unread cases, plays `ui_mail_deleted`, and displays unread-mail notification text. |
...
| `89` | `OP_ClearDataMsg` | `0x0091fe32` serialize, `0x0091fe4a` deserialize; handler `0x0046c510` | Single `u8 data_type`; the handler asserts/logs if the byte is not `1`, then clears a 100-entry data/status table and resets cached status state. This is not the older source-order `OP_ESStatusMsg` label. v546 XML now has the one-byte row matching the server's raw `{0x01}` send after `OP_SendLatestRequestMsg`. |
| `90` | `OP_ESZoneInstanceStatusMsg` | `0x0091fe62` serialize, `0x0091fea9` deserialize; handler `0x00491e10` | `u8 status`, two `string16` values, and trailing `u16`. The handler stores the trailing `u16` in GameScene status state and forwards the status plus the two string fields to the instance-status UI/path. v546 XML now has a fixed-field row, and the root opcode override maps this name to recovered client id `90` instead of the PacketParser row `93`. |
...
| `124` | `OP_PlayerHouseDisplayStatusMsg` | `0x00920d7d` serialize, `0x00920e07` deserialize; handler `0x00473e70` | `string16`, `u32`, `u64`, `u32`, `u64`, `u32`, `string16`, `u8`. v546 XML now has the compact display-status row instead of relying only on the later 1096+ house-status body. |
| `125` | `OP_PlayerHouseCloseUIMsg` | `0x00920e95` serialize, `0x00920ef3` deserialize; handler `0x00473e90` | `u32`, `u8 flag`; when flag is zero, an extra `string16` and `u8` follow. v546 XML now represents the conditional close-UI tail with `IfVariableNotSet`. |
...
| `142` | `OP_HouseCustomizationScreenMsg` | `0x0092b322` serialize, `0x009449f5` deserialize; handler `0x004a6530` | `u8 count`, then `count` entries of `u32` plus `string16`; the handler applies changed entries against the GameScene customization tree at `+0xec`. v546 XML now has the counted customization-list row. |
...
| `144` | `OP_CustomizationSetRequestMsg` | `0x00928b5f` serialize, `0x0094707f` deserialize; handler `0x00473ef0` | `string16`, `u32`, packed count, then entries of `u32`, `string16`, `u64`, `u32`, `u64`, `u32`, `u8`, `u8`; the handler routes it to the `InteriorCustomization` UI. v546 XML now has the packed customization-entry request row for future server handling/decoding. |
...
| `147` | `OP_CustomizationReplyMsg` | `0x00921482` serialize, `0x009214c7` deserialize | Handler-backed at `0x00473f10`: `u32`, `u32`, `u8`, routed to `Eq2GuiInteriorCustomizationWindow::update`. The last byte is widened to the reply code read at object `+0x0c`. v546 XML now has a fixed-field row, and the root opcode override maps this name to recovered client id `147` instead of the PacketParser row `145`. |
...
| `201` | `VeKnowledgebaseResponseMsg` / `OP_KnowledgebaseResponseMsg` | `0x00928f02` serialize, `0x009434fa` deserialize; handler `0x00473470` | `u32`, `u32 count`, entries of `string16`, `u32`, `string16`, then trailing `string16`. Deserializer rejects counts `>= 0x10001`. The handler forwards the payload to the `Help` UI. v546 XML now has the recovered counted article row instead of the older placeholder response shape. |
...
| `203` | `VeCSTicketInfoMsg` / `OP_CSTicketInfoMsg` | `0x00928f97` serialize, `0x00944e69` deserialize; handler `0x00473490` | `u32`, `u32 count`, entries of three `u32`, `string16`, three `u32`, `string16`. Deserializer rejects counts `>= 0x401`. The handler forwards the ticket list to the `Help` UI. v546 XML now has the counted ticket-info row. |
...
| `205` | `VeCSTicketCommentResponseMsg` / `OP_CSTicketCommentResponseMsg` | `0x00922764` serialize, `0x00947319` deserialize; handler `0x004734b0` | Two `u32`, `u32 count`, entries of `u8`, `u32`, `string16`, `string16`. Deserializer rejects counts `>= 0x81`. The handler forwards the comment list to the `Help` UI. v546 XML now has the counted comment-response row. |
...
| `209` | `OP_CSTicketChangeNotificationMsg` | `0x00922999` serialize, `0x009229de` deserialize; handler `0x004734d0` | `u32`, `u8`, `u32`. The handler forwards the compact notification to the `Help` UI. v546 XML now has the fixed three-field notification row. |
...
| `219` | `OP_UpdateClientPredFlagsMsg` | `0x00922dc1` serialize, `0x00922df7` deserialize; handler `0x0046c5f0` | `u32`, `u8`. The handler uses the trailing flag to toggle local player/client prediction state. v546 `WS_ServerControlFlags` now serializes the value as one byte instead of the legacy `int32`, and the root opcode override maps this name to recovered client id `219` instead of the PacketParser row `218`. |
| `220` | likely `OP_CSToolsRequestMsg` | `0x00922e31` serialize, `0x00922e51` deserialize | Single `u8 toggle`, matching the existing `WS_CSToolsRequest` row and PacketParser/source adjacency. Do not label this as the v546 `OP_ChangeServerControlFlagMsg`; the DoF server-control send path uses type `219` (`OP_UpdateClientPredFlagsMsg`) for the classic control flag body, while later clients keep the separate `OP_ChangeServerControlFlagMsg` XML row. |
...
| `236` | `OP_DisplayWarningMsg` | `0x00923562` serialize, `0x009235fd` deserialize; handler `0x00474e50` | `u8`, two `u32`, `string16`, three `u8`, two `u16`, and trailing `u32`. The handler routes the payload to `0x00687520`; do not label this row as `OP_EncounterBrokenMsg`. v546 XML now has a fixed-field row, and the root opcode override maps this name to recovered client id `236` instead of the PacketParser row `238`. |
...
| `277` | likely `OP_MailCommitSendMessageMsg` | `0x009253d5` serialize, `0x0092541e` deserialize | `u64`, `u32`, `string16`, `u32`. v546 XML now has a fixed-field row. |
| `278` | likely `OP_MailSendSystemMessageMsg` | `0x00925467` serialize, `0x009254c2` deserialize | Four `string16` values, `u64`, then the shared mail attachment payload (`coins`, `u16`, direct item-detail variant 0, bounded raw blob). v546 XML now exposes the fixed string/u64 prefix and the represented attachment metadata/footer fields; the raw item-detail blob is still part of the shared item-helper follow-up. |
| `279` | likely `OP_MailRemoveAttachFromMailMsg` | `0x0092551d` serialize, `0x00925557` deserialize | `string16`, `u32`, `u32`. v546 XML now has a fixed-field row. |
...
| `284` | likely `OP_CharNameChangedMsg` | `0x0092422a` serialize, `0x00924280` deserialize | Two `string16` values followed by three `u32` values. v546 XML now has a fixed-field row, and the root opcode override maps this name to recovered client id `284` instead of the PacketParser row `325`. |
...
| `288` | likely `OP_RequestGuildMembershipMsg` | `0x00924368` serialize, `0x00924395` deserialize | Two `u32` values. v546 XML now has a fixed-field row, and the root opcode override maps this name to recovered client id `288` instead of the PacketParser row `329`. |
...
| `301` | `OP_LFGUpdateMsg` | `0x00924630` serialize, `0x0092465d` deserialize; factory `0x0091cb7f`, type-info `0x00916da6`; handler `0x00473510` | Two `u8` values. The handler resolves the `GroupMembers` UI and applies the state through its virtual update method. v546 XML now has a fixed-field row, and the root opcode override maps this name to recovered client id `301` instead of the PacketParser row `343`. |
...
| `308` | likely `OP_ConsignViewGetPageMsg` | `0x0092482a` serialize, `0x00924884` deserialize | `u64`, four `u32` values. v546 XML now has a fixed-field row, and the root opcode override maps this name to recovered client id `308` instead of the PacketParser row `350`. |
| `309` | likely `OP_ConsignViewReleaseMsg` | `0x009248de` serialize, `0x0092490b` deserialize | `u64`, `u32`. v546 XML now has a fixed-field row, and the root opcode override maps this name to recovered client id `309` instead of the PacketParser row `351`. |
| `310` | likely `OP_ConsignRemoveItemsMsg` | `0x00924a3c` serialize, `0x00924a78` deserialize | `u64`, two `u32` values. v546 XML now has a fixed-field row, and the root opcode override maps this name to recovered client id `310` instead of the PacketParser row `352`. |
| `311` | `OP_UpdateDebugRadiiMsg` | `0x00924938` serialize, `0x009249ba` deserialize; factory `0x00927ee9`, type-info `0x0091ccc5` | Six `u32` values, raw `0x0c` block, trailing `u32`. v546 XML now has a fixed-field row, and the root opcode override maps this name to recovered client id `311` instead of the PacketParser row `353`. |
...
| `313` | `OP_ReportMsg` | `0x009192d0` serialize, `0x009192f9` deserialize; factory `0x00939968`, type-info `0x004b7a00` | Two `string16` values. v546 XML now has a fixed-field row, and the root opcode override maps this name to recovered client id `313` instead of the PacketParser row `355`. |
...
| `316` | `OP_ConsignViewSortMsg` | `0x00924c16` serialize, `0x00924c7e` deserialize; factory `0x0091cc42`, type-info `0x00916e8d` | `u64`, `u32`, `u32`, `u64`, `u32`, `u32`. v546 XML now has a fixed-field row, and the root opcode override maps this name to recovered client id `316` instead of the PacketParser row `357`. |
...
| `319` | `OP_ClientInDeathRegionMsg` | `0x00917916` serialize, `0x0091791b` deserialize; factory `0x0091cdb4`, type-info `0x0050ae10` | Stub serializer/deserializer; no custom payload. v546 XML now has an empty row, and the root opcode override maps this name to recovered client id `319` instead of the PacketParser row `360`. |
| `320` | `OP_CampClientMsg` | `0x00924dbf` serialize, `0x00924dd7` deserialize; factory `0x0091cdca`, type-info `0x00916fca` | Single `u32` field. v546 XML now has a fixed-field row, and the root opcode override maps this name to recovered client id `320` instead of the PacketParser row `361`. |
| `321` | `OP_CSToolAccessResponseMsg` | `0x00924e1f` serialize, `0x00924e4c` deserialize; factory `0x0091ce44`, type-info `0x00917034` | `u32`, trailing `u8` response/flag. v546 XML now has a fixed-field row, and the root opcode override maps this name to recovered client id `321` instead of the PacketParser row `362`. |
| `322` | `OP_DeleteGuildMsg` | `0x00924def` serialize, `0x00924e07` deserialize; factory `0x0091ce07`, type-info `0x00916fff` | Single `u32` field. The legacy source oplist keeps this row between `OP_CSToolAccessResponseMsg` and `OP_TrackingUpdateMsg`; PacketParser v546 metadata does not keep it in this range. v546 XML and the root opcode override now use recovered client id `322`, leaving the older PacketParser row `112` alone. |
...
records. `PacketStruct::GetOpcodeValue` also overrides confirmed v546 root ids
including `OP_CharNameChangedMsg` (`284`), `OP_ReloadLocalizedTxtMsg` (`287`),
`OP_RequestGuildMembershipMsg` (`288`), `OP_CsCategoryResponseMsg` (`299`),
`OP_MapFogDataInitMsg` (`328`), and
...
wire send path.
| `330` | `OP_CloseGroupInviteWindowMsg` | `0x00917934` serialize, `0x00917939` deserialize; factory `0x0091cf3d`, type-info `0x0091711b` | Handler-backed at `0x0046c4e0`; empty payload closes the cached group-invite window object and resets the stored window handle. v546 XML now has an empty row, and the root opcode override maps this name to recovered client id `330` instead of the PacketParser row `371`. |
...
| `333` | `OP_MailEventNotificationMsg` | `0x00924fd7` serialize, `0x0092501c` deserialize; factory `0x0093999a`, type-info `0x0093500e` | `u8` followed by three `string16` values. v546 XML now has a fixed-field row, and the root opcode override maps this name to recovered client id `333` instead of the PacketParser row `384`. |
...
| `336` | `OP_DisplayMailScreenMsg` | `0x00925678` serialize, `0x00925690` deserialize; factory `0x0091d0c4`, type-info `0x009172de` | Handler-backed at `0x0046cd10`; single `u32` body. The handler logs the display-mail-screen message. v546 XML now has a fixed-field row, and the root opcode override maps this name to recovered client id `336` instead of the PacketParser row `377`. |
...
| `366` | `OP_AuctionCharacterReply` | `0x0092634b` serialize, `0x00926396` deserialize; factory `0x0091d232`, type-info `0x00917500`; handler `0x00475c30` | `u8`, `u32`, `u32`, `u64`. The handler uses the status byte for Station Exchange character-transfer/auction error text and clears/refreshes the `Exchange` UI when present. v546 XML now has a fixed-field row, and the root opcode override maps this name to recovered client id `366` instead of the PacketParser row `407`. |
